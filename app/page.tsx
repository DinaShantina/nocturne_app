...
import { useActionState, useEffect, useState, useMemo, startTransition, useRef } from "react"; 
...
const NocturneMap = dynamic(() => import("./components/NocturneMap"), { 
  ssr: false,
  loading: () => <div className="h-[400px] w-full bg-zinc-900 animate-pulse rounded-2xl flex items-center justify-center font-mono text-xs uppercase text-white/20">Booting Map...</div>
});
...
const NocturneCalendar = ({ value, onChange }: { value: string, onChange: (val: string) => void }) => {
  ...
  return (
    ...
    >
      <div className="bg-black/60 border border-white/10 rounded-xl px-4 py-3 text-xs font-mono cursor-pointer flex justify-between items-center group-hover:border-teal-500 transition-colors">
        ...
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-teal-500" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
      </div>
      <input 
        ref={inputRef}
        type="date" 
...
interface Stamp {
  id: string; city: string; country: string; venue: string; activity: string;
  date: string; color: string; points: number; image?: string; lat?: number; lng?: number;
  category?: string; 
}

const CATEGORIES = ["TECHNO", "ART", "JAZZ", "DINING", "LOUNGE"];
...
export default function Home() {
  ...
  const [activeCityFilters, setActiveCityFilters] = useState<Record<string, string[]>>({});
  const [showForm, setShowForm] = useState(false);
  const [countrySortOrder, setCountrySortOrder] = useState<'ALPHA' | 'RECENT'>('ALPHA');
...
const normalizedCountry = (countryName: string) => {
  ...
  navigator.geolocation.getCurrentPosition(async (position) => {
    try {
      ...
      if (form) {
        (form.querySelector('input[name="city"]') as HTMLInputElement).value = (data.city || data.locality || "").toUpperCase();
...
(form.querySelector('input[name="country"]') as HTMLInputElement).value = normalizedCountry.toUpperCase();      
        // Suggest nearest landmark/venue if available
        if (data.lookupSource === "coordinates" && data.locality) {
            (form.querySelector('input[name="venue"]') as HTMLInputElement).placeholder = `SUGGESTION: ${data.locality.toUpperCase()}`;
        }
      }
    } catch (error) {
  ...
  const uniqueCities = useMemo(() => new Set(stamps.map(s => s.city.toUpperCase())).size, [stamps]);
  ...
  try {
    ...
    const docRef = await addDoc(collection(db, "stamps"), {
      ...
      lat: lat, 
      lng: lng,
      color: ["#22d3ee", "#818cf8", "#f472b6", "#fbbf24", "#34d399"][Math.floor(Math.random() * 5)],
  ...
  const fileInput = editFormRef.current?.querySelector('input[name="newImage"]') as HTMLInputElement;
  let file = fileInput?.files?.[0];
  let imageUrl = selectedStamp.image || "";

  try {
    if (file && file.size > 0) {
      file = await imageCompression(file, { maxSizeMB: 0.8, maxWidthOrHeight: 1200 });
      const storageRef = ref(storage, `stamps/${Date.now()}_${file.name}`);
...
// 1. Ensure your state is defined (usually at the top of the component)
const [activeCategory, setActiveCategory] = useState('ALL');

// 2. The filtering logic (placed before groupedStamps)
...
const groupedStamps = useMemo(() => {
  // 1. Group the stamps
  const groups = filteredStamps.reduce((acc, stamp) => {
  ...
  if (countrySortOrder === 'ALPHA') {
    // Alphabetical: A -> Z
    sortedCountryNames = Object.keys(groups).sort((a, b) => a.localeCompare(b));
  } else {
    // Recent: Look at the newest stamp in each country and sort countries by that
    sortedCountryNames = Object.keys(groups).sort((a, b) => {
      const newestA = Math.max(...groups[a].map(s => new Date(s.date || 0).getTime()));
      const newestB = Math.max(...groups[b].map(s => new Date(s.date || 0).getTime()));
  ...
  sortedCountryNames.forEach(name => {
    ...
    orderedGroups[name] = [...groups[name]].sort((a, b) => 
...
return (
  <>
    {/* 1. OVERLAY BACKDROP - Stays outside the main tag */}
    <div 
      className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] transition-opacity duration-500  ${showSidebar ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      onClick={() => setShowSidebar(false)}
    />
    {/* 2. SLIDE-OUT SIDEBAR */}
<aside 
  className={`fixed left-0 top-0 h-full bg-zinc-950 border-r border-white/10 z-[9999] transition-all duration-500
    ${galleryIndex !== null ? '-translate-x-full' : ''} // Hide completely when gallery is open
    ${showSidebar ? 'w-80 translate-x-0' : 'w-80 -translate-x-[calc(100%-64px)]'}`}
>
      {/* PEEK AREA (Always visible strip) */}
      {!showSidebar && (
        <div 
        ...
        >
          <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-teal-500 to-purple-600 p-[2px]">
            <div className="w-full h-full rounded-full bg-black flex items-center justify-center overflow-hidden">
              <Image src="/logo.png" alt="Profile" width={24} height={24} className="opacity-80" />
            </div>
          </div>
          <div className="h-[1px] w-4 bg-white/10" />
          <p className="text-[9px] font-mono text-zinc-500 [writing-mode:vertical-lr] uppercase tracking-widest">Dashboard</p>
        </div>
      )}

      {/* FULL CONTENT (Visible when open) */}
      <div className={`flex-1 flex flex-col p-8 transition-opacity duration-300 ${showSidebar ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        
        {/* Profile Header */}
        <div className="flex justify-between items-start mb-12">
           <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-zinc-900 border border-white/10 p-2">
                 <Image src="/logo.png" alt="User" width={40} height={40} className="object-contain" />
              </div>
              <div>
                <p className="text-xs font-black italic uppercase tracking-widest text-white">Urban Agent</p>
                <p className="text-[9px] font-mono text-purple-500/80 uppercase font-bold tracking-tight">Access Level II</p>
              </div>
           </div>
           <button onClick={() => setShowSidebar(false)} className="text-zinc-600 hover:text-white transition-colors">
        ...
        <div className="space-y-12">
          ...
          <div className="space-y-2">
            <p className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest">Lifetime Exploration</p>
            <div className="flex items-baseline gap-2">
              <h3 className="text-5xl font-black italic uppercase tracking-tighter text-white">
                {totalDistance.toLocaleString()}
              </h3>
              <span className="text-[10px] font-mono text-zinc-600">KM</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-8">
            <div className="space-y-1">
              <p className="text-[8px] font-mono text-zinc-500 uppercase tracking-[0.2em]">Stamps</p>
            ...
            <div className="space-y-1 border-l border-white/5 pl-4">
              <p className="text-[8px] font-mono text-zinc-500 uppercase tracking-[0.2em]">Cities Visited</p>
              <p className="text-2xl font-black italic uppercase tracking-tighter text-white">
          ...
          <div className="pt-4">
            <p className="text-[9px] font-mono text-zinc-600 uppercase mb-3 tracking-widest">Influence Rank</p>
            <div className="inline-flex items-center gap-2 bg-purple-500/10 border border-purple-500/20 px-4 py-2 rounded-xl">
              ...
              <p className="text-[10px] font-black italic uppercase text-purple-400 tracking-widest">
                {stamps.length > 15 ? 'Resident' : 'Ghost'}
              </p>
            </div>
          </div>
        </div>
  ...
  <div className="relative w-32 h-32 transition-all duration-500 rounded-full flex items-center justify-center overflow-hidden bg-white shadow-xl dark:bg-transparent dark:shadow-none">
    ...
    <div className="relative w-full h-full">
      ...
      <div className="opacity-0 dark:opacity-100 transition-opacity duration-500 absolute inset-0">
        ...
        <h1 className="text-3xl font-black tracking-[0.4em] uppercase italic">Nocturne</h1>
        <p className="text-[10px] font-mono text-teal-500 mt-2 tracking-[0.3em] uppercase">Cultural Passport</p>
        
        <div className="flex gap-8 mt-8 border-t border-black/5 dark:border-white/5 pt-8">
            <div className="text-center">
                <p className="text-[9px] font-mono text-black/30 dark:text-white/30 tracking-[0.2em] mb-1">TOTAL STAMPS</p>
                <p className="text-2xl font-black italic">{totalStamps.toString().padStart(2, '0')}</p>
            </div>
            <div className="w-[1px] h-10 bg-black/10 dark:bg-white/10"></div>
            <div className="text-center">
                <p className="text-[9px] font-mono text-black/30 dark:text-white/30 tracking-[0.2em] mb-1">CITIES VISITED</p>
                <p className="text-2xl font-black italic">{uniqueCities.toString().padStart(2, '0')}</p>
            </div>
        </div>
  ...
  {!showForm ? (
    ...
    <div className="w-full max-w-xl animate-in fade-in slide-in-from-top-4 duration-500">
       <div className="flex justify-end mb-2">
         <button onClick={() => setShowForm(false)} className="text-[10px] font-mono opacity-50 hover:opacity-100">CLOSE [X]</button>
       </div>
           {/* Entry Form */}
       <form ref={formRef} onSubmit={(e) => handleCreateStamp(e)}
              className="w-full max-w-md space-y-4 mx-auto mb-12 relative z-10 p-8 rounded-[2.5rem] 
                        ...
                        dark:bg-white/[0.03] dark:border-white/10 dark:shadow-none transition-all duration-500 border"
            >
              <div className="flex flex-col gap-2">
                ...
                >
                  <span className="relative z-10 flex items-center justify-center gap-2">
                    {isLocating ? "Scanning Environment..." : "📍 Detect Location"}
                  </span>
                </button>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <NocturneCalendar value={createDate} onChange={setCreateDate} />
              </div>
              ...
              <div className="space-y-2">
                <div className="relative">
                  <input type="file" name="image" accept="image/*" onChange={(e) => setImageUploaded(!!e.target.files?.length)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                  <div className={`w-full border border-dashed rounded-xl py-4 text-center text-[10px] font-mono flex items-center justify-center gap-2 uppercase transition-all
                    ${imageUploaded 
                    ...
                    {imageUploaded ? "Evidence Attached" : "Upload Evidence"}
                  </div>
                </div>
              </div>
      ...
      <div className="w-full max-w-5xl mb-12 flex flex-wrap justify-center gap-4">
        <button onClick={() => setFilter("ALL")} className={`px-6 py-2 rounded-full text-[10px] font-black border transition-all ${filter === "ALL" ? 'bg-white text-black border-white' : 'border-white/10 text-white/40 hover:border-white/30'}`}>ALL STAMPS</button>
        {CATEGORIES.map(c => (
          <button key={c} onClick={() => setFilter(c)} className={`px-6 py-2 rounded-full text-[10px] font-black border transition-all ${filter === c ? 'bg-teal-500 text-black border-teal-500' : 'border-white/10 text-white/40'}`}>{c}</button>
        ))}
      </div>
...
<div className="w-full max-w-7xl mb-20">
  ...
  {view === 'passport' ? (
    ...
    {Object.entries(groupedStamps).map(([country, items]) => {
      const uniqueCitiesInCountry = Array.from(new Set(items.map(s => s.city.toUpperCase())));
      const currentFilters = activeCityFilters[country] || [];
      const filteredItems = items.filter(s => 
        currentFilters.length === 0 || currentFilters.includes(s.city.toUpperCase())
      );

      return (
        /* ADD THE ID HERE FOR SCROLLING */
        <section id={`section-${country.toLowerCase()}`} key={country} className="mb-24 relative isolate scroll-mt-24">
...
<div className="flex justify-between items-end w-full border-b border-purple-200/20 dark:border-white/5 pb-6">
  <div className="flex flex-col">
    ...
    <div className="flex items-center gap-3 mt-2">
      ...
      <p className="text-[10px] font-mono text-purple-500 dark:text-teal-400 font-bold tracking-[0.1em]">
        {filteredItems.length.toString().padStart(2, '0')} STAMPS
      </p>
    </div>
  </div>
  ...
  <div className="flex gap-4 mb-1">
    <button 
      onClick={() => scrollRefs.current[country]?.scrollBy({ left: -300, behavior: 'smooth' })} 
      ...
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
        <path d="M9 18l6-6-6-6"/>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {/* "ALL" Button */}
              <button
                onClick={() => setActiveCityFilters(prev => ({ ...prev, [country]: [] }))}
         ...
         <div ref={el => { scrollRefs.current[country] = el }} className="flex overflow-x-auto pt-10 gap-12 pb-12 px-10 no-scrollbar scroll-smooth min-h-[300px]">
            {filteredItems.map((s) => {
              // 1. Logic for the "Just Issued" badge
              const stampTime = s.createdAt?.seconds ? s.createdAt.seconds * 1000 : s.createdAt;
              const isJustIssued = stampTime && (Date.now() - stampTime < 60000);

              return (
                <div 
                  key={s.id} 
                  id={`stamp-${s.id}`} // <--- Target for the auto-scroll
                  onClick={() => { setSelectedStamp(s); setEditDate(s.date); setIsEditing(false); }} 
                  className={`flex-none w-64 h-64 group/stamp relative rounded-full flex items-center justify-center cursor-pointer transition-all duration-500 hover:scale-110 active:scale-95 shadow-2xl ${isJustIssued ? 'z-[100]' : 'z-10 hover:z-50'}`}      >
                  {/* 2. The Badge UI */}
                  {isJustIssued && (
                  ...
                  <div className="absolute inset-0 rounded-full border-[6px]" style={{ borderColor: s.color }}></div>
                  <div className="absolute inset-[6px] rounded-full overflow-hidden z-0 bg-zinc-950">
                    {s.image && <Image src={s.image} alt={s.venue} fill className="object-cover grayscale group-hover/stamp:grayscale-0 transition-all duration-700" />}
                  </div>
                  <div className="relative z-10 text-center px-6">
                    <p className="text-xl font-black uppercase italic text-white drop-shadow-md">{s.venue}</p>
                    <p className="text-[10px] font-mono text-white/60">{s.city}</p>
                  </div>
                </div>
              );
            })}
    ...
    <NocturneMap 
      stamps={filteredStamps} onSelectLocation={() => setView('passport')} onCitySelect={handleMapCitySelect}
    />
  )}
</div>
      ...
      {selectedStamp && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="w-full max-w-lg bg-zinc-900 border border-white/10 rounded-[40px] overflow-hidden shadow-2xl relative">
            <button onClick={() => setSelectedStamp(null)} className="absolute top-6 right-6 z-50 text-white/40 hover:text-white transition-colors">
          ...
          <div className="relative h-64 w-full bg-black overflow-hidden group/zoom">
            {selectedStamp.image ? (
              <>
                <Image 
                  ...
                  onClick={() => {
                    if (!showRedactConfirm) {
                      ...
                      const index = stamps.findIndex(s => s.id === selectedStamp.id);
                ...
                {!showRedactConfirm && (
                  <button 
                    onClick={(e) => { e.stopPropagation(); setShowRedactConfirm(true); }}
                  ...
                  >
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest">Redact Evidence</span>
                  </button>
                )}
                ...
                {showRedactConfirm && (
                  <div className="absolute inset-0 z-[60] flex flex-col items-center justify-center bg-red-950/20 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                    <p className="text-[10px] font-black text-white tracking-[0.3em] uppercase mb-4 drop-shadow-md">Confirm Redaction?</p>
                    <div className="flex gap-3">
                      <button 
                        onClick={() => { handleDeleteImage(); setShowRedactConfirm(false); }}
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 via-transparent to-transparent pointer-events-none"></div>
                ...
                <div className="p-10 -mt-12 relative z-10">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="px-3 py-1 bg-teal-500 text-black text-[9px] font-black rounded-full uppercase tracking-widest">{selectedStamp.category || "GENERAL"}</span>
                    <span className="text-[10px] font-mono text-white/40">{selectedStamp.date.replace(/-/g, '.')}</span>
                  </div>
                  <h2 className="text-4xl font-black uppercase italic tracking-tighter mb-2" style={{ color: selectedStamp.color }}>{selectedStamp.venue}</h2>
                  <p className="text-sm font-mono text-white/60 mb-6 uppercase tracking-widest">{selectedStamp.city}, {selectedStamp.country}</p>
                  <div className="bg-white/5 border border-white/5 p-6 rounded-2xl mb-8"><p className="text-xs font-mono leading-relaxed text-white/80 italic">"{selectedStamp.activity}"</p></div>
                  <div className="flex gap-4">
                    <button onClick={() => setIsEditing(true)} className="flex-1 py-4 bg-white text-black font-black rounded-2xl text-[10px] uppercase hover:bg-teal-400 transition-all">Edit Entry</button>
                    <button onClick={() => setSelectedStamp(null)} className="px-8 py-4 border border-white/10 rounded-2xl text-[10px] font-black uppercase hover:bg-white/5 transition-all text-white/40">Close</button>
                  </div>
                </div>
              </div>
            ) : (
              <div className="p-10 pt-16"> {/* Increased padding for the close button */}
                <h2 className="text-xl font-black uppercase tracking-widest mb-8 text-teal-400 italic">Modify Log</h2>
                <form ref={editFormRef} onSubmit={handleUpdate} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <input name="city" defaultValue={selectedStamp.city} placeholder="CITY" required className="bg-black border border-white/10 rounded-xl px-4 py-3 text-xs font-mono uppercase outline-none focus:border-teal-500" />
                    <input name="country" defaultValue={selectedStamp.country} placeholder="COUNTRY" required className="bg-black border border-white/10 rounded-xl px-4 py-3 text-xs font-mono uppercase outline-none focus:border-teal-500" />
                  </div>
                  <input name="venue" defaultValue={selectedStamp.venue} placeholder="VENUE" required className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-xs font-mono uppercase outline-none focus:border-teal-500" />
                  <div className="grid grid-cols-2 gap-4">
                    <select name="category" defaultValue={selectedStamp.category} className="bg-black border border-white/10 rounded-xl px-4 py-3 text-xs font-mono uppercase outline-none focus:border-teal-500">
                      <option value="">NO CATEGORY</option>
                      {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                    <NocturneCalendar value={editDate || selectedStamp.date} onChange={setEditDate} />
                  </div>
                  <textarea name="activity" defaultValue={selectedStamp.activity} rows={3} placeholder="ACTIVITY" className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-xs font-mono outline-none focus:border-teal-500" />
                  
                  <div className="relative">
                    <input type="file" name="newImage" accept="image/*" onChange={(e) => setEditImageUploaded(!!e.target.files?.length)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                    <div className={`w-full bg-black/40 border border-dashed rounded-xl py-4 text-center text-[10px] font-mono flex items-center justify-center gap-2 uppercase ${editImageUploaded ? 'border-teal-500 text-teal-500' : 'border-white/10 text-gray-500'}`}>
                      {editImageUploaded ? "New Evidence Ready" : "Change Image (Optional)"}
                    </div>
                  </div>

                  <div className="flex gap-3 pt-6">
                    <button type="button" onClick={() => setShowDeleteConfirm(selectedStamp.id)} className="p-4 bg-red-500/10 text-red-500 rounded-2xl border border-red-500/20 hover:bg-red-500 hover:text-white transition-all">
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M3 6h18M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
                    </button>
                    <button type="submit" className="flex-1 py-4 text-[10px] font-black uppercase bg-white text-black rounded-2xl hover:bg-teal-400 transition-colors shadow-lg">Save Update</button>
                    <button type="button" onClick={() => setIsEditing(false)} className="flex-1 py-4 text-[10px] font-black uppercase border border-white/10 rounded-2xl hover:bg-white/5 transition-colors text-white/40">Back</button>
                  </div>
                </form>
              </div>
            )}
        ...
        {galleryIndex !== null && (
          <div className="fixed inset-0 z-[300] bg-black/98 backdrop-blur-2xl flex items-center justify-center animate-in fade-in duration-300">
            {/* Navigation HUD */}
            <button onClick={() => setGalleryIndex(null)} className="absolute top-10 right-10 z-[310] text-white/40 hover:text-white transition-colors">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M18 6L6 18M6 6l12 12"/></svg>
            </button>

            <button onClick={prevImage} className="absolute left-6 md:left-12 p-4 text-white/20 hover:text-teal-400 transition-all z-[310]">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><path d="M15 18l-6-6 6-6"/></svg>
            </button>
            <button onClick={nextImage} className="absolute right-6 md:right-12 p-4 text-white/20 hover:text-teal-400 transition-all z-[310]">
            ...
            <div className="relative w-[90vw] h-[80vh] flex items-center justify-center">
              {stamps[galleryIndex]?.image ? (
                ...
                <div className="text-zinc-800 text-6xl font-black italic opacity-20">NO IMAGE DATA</div>
              )}
            </div>

            {/* Nocturne HUD */}
            <div className="absolute bottom-12 left-12 text-left pointer-events-none">
              <p className="text-teal-500 font-mono text-[10px] tracking-[0.5em] uppercase mb-2">Evidence Log #{galleryIndex + 1}</p>
              <h3 className="text-4xl font-black italic uppercase text-white tracking-tighter">{stamps[galleryIndex]?.venue}</h3>
              <p className="text-white/40 font-mono text-xs uppercase">{stamps[galleryIndex]?.city} // {stamps[galleryIndex]?.date.replace(/-/g, '.')}</p>
            </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/98 backdrop-blur-md">
          <div className="bg-zinc-900 border border-red-500/20 p-10 rounded-[40px] text-center max-w-xs">
            <h2 className="text-xl font-black uppercase mb-2">Delete Stamp?</h2>
            <p className="text-[10px] font-mono text-white/40 mb-8 tracking-tighter">This action cannot be undone.</p>
            <div className="flex flex-col gap-2 mt-8">
              <button onClick={async () => { await deleteDoc(doc(db, "stamps", showDeleteConfirm)); setShowDeleteConfirm(null); setSelectedStamp(null); }} className="w-full py-4 bg-red-500 text-white font-black rounded-2xl text-[10px] uppercase">Permanent Delete</button>
              <button onClick={() => setShowDeleteConfirm(null)} className="w-full py-4 bg-white/5 text-gray-400 font-black rounded-2xl text-[10px] uppercase border border-white/10">Keep Entry</button>
            </div>
          </div>
        </div>
      )}
          ...
          className="flex items-center gap-2 px-4 py-2 bg-zinc-900 border border-white/10 rounded-full hover:border-teal-500 transition-all active:scale-95"
        >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-teal-500">
            ...
            <button onClick={() => setView('passport')} className={`text-[10px] font-black tracking-widest transition-all ${view === 'passport' ? 'text-teal-400 scale-110' : 'text-gray-500 hover:text-white'}`}>PASSPORT</button>
            <button onClick={() => {
    setView('map');
  ...
  }}  className={`text-[10px] font-black tracking-widest transition-all ${view === 'map' ? 'text-teal-400 scale-110' : 'text-gray-500 hover:text-white'}`}>GLOBAL MAP</button>
